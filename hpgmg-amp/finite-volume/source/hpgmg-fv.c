...
//    double minTime   = 60.0; // minimum time in seconds that the benchmark should run
    ...
    int maxSolves = 10000; // but don't go crazy in the small cases
    ...
    if(doTiming==1){
      ...
      minSolves = (minSolves > maxSolves) ? maxSolves : minSolves;
    }
    #else
    double minTime   = 5.0; // minimum time in seconds that the benchmark should run
    int maxSolves = 10000; // but don't go crazy in the small cases
    ...
    #else
    if(doTiming==0){
        timePerSolve = all_grids->timers.MGSolve/numSolves;
    }
    #endif
