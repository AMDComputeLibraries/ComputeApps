...
int main(int argc, char** argv)
{
   ...
   OclSimSoa* oclSimSoa = initOclSimSoa(sim, cmd);
   ...
   for (; iStep<nSteps;)
   {
      ...
      printThings(sim, iStep, getElapsedTime(oclTimestep));
      printTensor(iStep, sim->defInfo->stress);
      
      startTimer(timestepTimer);

      computeIterationSoa2(sim, oclSimSoa);
...
#endif
   stressOut = fopen("stressOut.txt","w");
