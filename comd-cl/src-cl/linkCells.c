...
/// Further modified for OpenCL so that Inner cells are first, followed by 
/// 'skin' cells and then halo cells.
...
int getBoxFromTuple(LinkCell* boxes, int ix, int iy, int iz)
{
   ...
   int innerSize[3];
   innerSize[0] = gridSize[0]-2;
   innerSize[1] = gridSize[1]-2;
   innerSize[2] = gridSize[2]-2;
   ...
   {
      iBox = boxes->nInnerBoxes + 2*innerSize[2]*innerSize[1] + 2*innerSize[2]*(innerSize[0]+2) +
         (innerSize[0]+2)*(innerSize[1]+2) + (innerSize[0]+2)*(iy+0) + (ix+0);
   }
   // Skin in Z-
   else if (iz == 0)
   {
      iBox = boxes->nInnerBoxes + 2*innerSize[2]*innerSize[1] + 2*innerSize[2]*(innerSize[0]+2) +
         (innerSize[0]+2)*(iy+0) + (ix+0);
   }
   // Skin in Y+
   else if (iy == gridSize[1]-1)
   {
      iBox = boxes->nInnerBoxes + 2*innerSize[2]*innerSize[1] + innerSize[2]*(innerSize[0]+2) +
         (innerSize[0]+2)*(iz-1) + (ix+0);
   }
   // Skin in Y-
   else if (iy == 0)
   {
      iBox = boxes->nInnerBoxes + 2*innerSize[2]*innerSize[1] + (iz-1)*(innerSize[0]+2) + (ix+0);
   }
   // Skin in X+
   else if (ix == gridSize[0]-1)
   {
      iBox = boxes->nInnerBoxes + innerSize[1]*innerSize[2] + (iz-1)*innerSize[1] + (iy-1);
   }
   // Skin in X-
   else if (ix == 0)
   {
      iBox = boxes->nInnerBoxes + (iz-1)*innerSize[1] + (iy-1);
   }

   // innerl link cell.
   else
   {
      iBox = (ix-1) + (innerSize[0])*(iy-1) + (innerSize[0])*(innerSize[1])*(iz-1);
   }
   assert(iBox >= 0);
...
void getTuple(LinkCell* boxes, int iBox, int* ixp, int* iyp, int* izp)
{
   ...
   // it's a skin box
   else if (iBox >= boxes->nInnerBoxes && iBox < boxes->nLocalBoxes)
   {
      int ink;
      ink = iBox - boxes->nInnerBoxes;
      if (ink < 2*innerSize[1]*innerSize[2])
      {
         if (ink < innerSize[1]*innerSize[2]) 
         {
            ix = 0;
         }
         else 
         {
            ink -= innerSize[1]*innerSize[2];
            ix = innerSize[0] + 1;
         }
         iy = 1 + ink % innerSize[1];
         iz = 1 + ink / innerSize[1];
      }
      else if (ink < (2 * innerSize[2] * (innerSize[1] + innerSize[0] + 2))) 
      {
         ink -= 2 * innerSize[2] * innerSize[1];
         if (ink < ((innerSize[0] + 2) *innerSize[2])) 
         {
            iy = 0;
         }
         else 
         {
            ink -= (innerSize[0] + 2) * innerSize[2];
            iy = innerSize[1] + 1;
         }
         ix = ink % (innerSize[0] + 2);
         iz = 1 + ink / (innerSize[0] + 2);
      }
      else 
      {
         ink -= 2 * innerSize[2] * (innerSize[1] + innerSize[0] + 2);
         if (ink < ((innerSize[0] + 2) * (innerSize[1] + 2))) 
         {
            iz = 0;
         }
         else 
         {
            ink -= (innerSize[0] + 2) * (innerSize[1] + 2);
            iz = innerSize[2] + 1;
         }
         ix = ink % (innerSize[0] + 2);
         iy = ink / (innerSize[0] + 2);
      }
      // These have zero offset
